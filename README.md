...
本番ビルドは共通の起動用資産を保存し、保存済みのノート・フォルダを閲覧専用で表示する。認証済みのアプリ起動時には、自分のMyDriveのフォルダとノート本文を順次先読みする。認証済み状態が維持されていれば、通信復帰・画面への復帰時にも再試行し、短時間の重複通知をまとめる。通常の画面表示・編集は維持し、共有されているだけの他人のノート本文は自動先読みしない。ブラウザの保存容量や通信状況によって取得・保持できない項目があるため、全件保存やバックアップを保証しない。

同じユーザーの背景先読みはWeb Locksでタブ間排他し、別タブが取得中ならその試行を省略する。Web Locksを利用できない環境では、背景先読みだけを省略し、通常閲覧とそのキャッシュ保存は維持する。

キャッシュ専用モードからの再認証、定期・変更後の再取得、別タブの認証変更検出、通常取得との同一要求統合、添付画像、容量回収、フォルダHTTP拒否の接続などは継続実装中。キャッシュ専用モードから通信を戻した場合は、現在は再読み込みで認証を確認し直す。仕様と実装済み範囲は区別し、最新の判断・検証は [意思決定台帳](docs/offline-pwa-decisions.md) を参照する。

...
Yjsのサーバー保存確認後の通知、別タブの変更・認証変更検出、通常取得との同一要求統合、添付画像、容量回収、フォルダHTTP拒否の接続などは継続実装中。既に認証済み・ゲストの画面を今回の復帰処理で再確認するものではない。仕様と実装済み範囲は区別し、最新の判断・検証は [意思決定台帳](docs/offline-pwa-decisions.md) を参照する。

...
キャッシュ専用モードからの再認証、定期・変更後の再取得、複数タブや通常取得との重複調整、添付画像、容量回収、フォルダHTTP拒否の接続などは継続実装中。キャッシュ専用モードから通信を戻した場合は、現在は再読み込みで認証を確認し直す。仕様と実装済み範囲は区別し、最新の判断・検証は [意思決定台帳](docs/offline-pwa-decisions.md) を参照する。

...
test("verified startup Alice to Bob removes Alice before publishing Bob", async ({
  ...
  await page.route("**/api/me", (route) =>
    route.fulfill({
      ...
      json: {
        user: { id: "bob", email: "bob@example.test", displayName: "Bob" },
      },
    }),
  );
...
test("verified checks preserve the same viewer buffer and switch away from Alice in both tabs", async ({
  ...
  await context.route("**/api/**", (route) => {
    ...
    if (path === "/api/me") {
      ...
      return route.fulfill({
        ...
        json: { user: { id: actor, email: `${actor}@example.test`, displayName: actor } },
      });
    }
    return route.fulfill({ headers: { "X-MiyulabMD-Session-User": `user:${actor}` }, json: [] });
  });
  await page.goto("/tests/browser/fixtures/app-shell.html");
  ...
  await expect(page.getByLabel("Unsent viewer buffer")).toHaveValue("do not discard");
...
test("explicit logout blocks old UI in both tabs, clears captured and confirmed actors, and uses native GET", async ({
  ...
  await context.route("**/api/**", async (route) => {
    const path = new URL(route.request().url()).pathname;
    const headers = { "X-MiyulabMD-Session-User": loggedOut ? "guest" : "user:alice" };
    if (path === "/api/me") {
      return route.fulfill({ headers, json: { user: loggedOut ? null : {
        id: "alice", email: "alice@example.test", displayName: "Alice",
      } } });
    }
    if (path === `/api/notes/${note.id}`) {
      noteRequests += 1;
      await late.promise;
      return route.fulfill({ headers: { "X-MiyulabMD-Session-User": "user:alice" }, json: note });
    }
    return route.fulfill({ headers, json: [] });
  });
  ...
  try {
    ...
    const peer = await context.newPage();
    await peer.goto("/tests/browser/fixtures/app-shell.html");
    await expect(peer.getByLabel("Viewer context")).toContainText('"id":"alice"');
    await page.evaluate(async (note) => {
      const cacheUrl = "/src/lib/offline-cache.ts";
      ...
      seedNoteCache(note);
      Object.assign(window, { lateMemoryRead: loadNote(note.id, true).catch(() => null) });
    }, note);
    await expect.poll(() => noteRequests).toBe(1);
    await page.getByRole("button", { name: "Alice", exact: true }).click();
    await page.getByRole("menuitem", { name: "ログアウト", exact: true }).click();
    await expect(page.getByLabel("Viewer context")).not.toContainText('"id":"alice"');
    await expect(peer.getByLabel("Viewer context")).not.toContainText('"id":"alice"');
    ...
    const caches = await page.evaluate(async (id) => {
      await (window as Window & { lateMemoryRead: Promise<unknown> }).lateMemoryRead;
      const cacheUrl = "/src/lib/offline-cache.ts";
      const memoryUrl = "/src/lib/note-cache.ts";
      const { openOfflineCache } = await import(cacheUrl);
...
test("identity clear prevents pending list and folder reads restoring memory", async ({ page }) => {
  ...
  await page.route("**/api/**", async (route) => {
    ...
    return route.fulfill({
      ...
      json: new URL(route.request().url()).pathname === "/api/notes"
        ? { notes: [note] } : { id: "folder-1", name: "private" },
    });
  });
  try {
    await page.goto("/tests/browser/fixtures/storage.html");
    await page.evaluate(async (note) => {
      ...
      Object.assign(window, {
        lateListReads: Promise.allSettled([loadNotes(true), loadFolder("folder-1")]),
      });
    }, note);
    ...
    const result = await page.evaluate(async () => {
      await (window as Window & { lateListReads: Promise<unknown> }).lateListReads;
      const url = "/src/lib/list-cache.ts";
      const { peekNotes, peekFolder } = await import(url);
      return { notes: peekNotes(), folder: peekFolder("folder-1") ?? null };
    });
    expect(result).toEqual({ notes: null, folder: null });
  } finally {
    release.resolve();
  }
});

for (const failure of ["server unreachable", "storage deletion fails"] as const) {
  test(`logout ${failure} stops local use and warns without claiming completion`, async ({ page, context }) => {
    let preparations = 0;
    let navigations = 0;
    await context.route("**/api/**", (route) => route.fulfill({
      headers: { "X-MiyulabMD-Session-User": "user:alice" },
      json: new URL(route.request().url()).pathname === "/api/me"
        ? { user: { id: "alice", email: "alice@example.test", displayName: "Alice" } } : [],
    }));
    await context.route("**/auth/logout", (route) => {
      if (route.request().method() === "GET") navigations += 1;
      else preparations += 1;
    ...
    for (const target of [page, peer]) {
      await expect(target.getByRole("alert")).toContainText("完了できませんでした");
      await expect(target.getByLabel("Viewer context")).not.toContainText('"id":"alice"');
      await target.evaluate(() => window.dispatchEvent(new Event("online")));
      await expect(target.getByLabel("Viewer context")).toContainText('"cacheViewerId":null');
    }

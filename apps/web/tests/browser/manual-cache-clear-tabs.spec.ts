...
test("durable device epoch rejects stale Alice and Bob handles when BroadcastChannel is missed", async ({
  ...
  await page.addInitScript(() => Object.defineProperty(globalThis, "BroadcastChannel", { value: undefined }));
  await page.goto("/tests/browser/fixtures/storage.html");
  const other = await context.newPage();
  await other.addInitScript(() => Object.defineProperty(globalThis, "BroadcastChannel", { value: undefined }));
  await other.goto("/tests/browser/fixtures/storage.html");
  try {
    await other.evaluate(async (source) => {
      const { captureOfflineCacheScope, openOfflineCache } = await import("/src/lib/offline-cache.ts");
      const alice = await openOfflineCache({ userId: "manual-tab-alice-7" });
      const bob = await openOfflineCache({ userId: "manual-tab-bob-7" });
      await alice.putNote({ ...source, id: "manual-tab-note-a-7", ownerId: "manual-tab-alice-7" });
      await bob.putNote({ ...source, id: "manual-tab-note-b-7", ownerId: "manual-tab-bob-7" });
    ...
    const stale = await other.evaluate(async (source) => {
      const fixture = window as typeof window & { alice: { putNote(n: unknown): Promise<void>; close(): void }; bob: { putNote(n: unknown): Promise<void>; close(): void }; aliceScope: unknown; bobScope: unknown };
      const alice = await fixture.alice.putNote({ ...source, id: "manual-tab-late-a-7" }).then(() => false, () => true);
      const bob = await fixture.bob.putNote({ ...source, id: "manual-tab-late-b-7" }).then(() => false, () => true);
      const scoped = await import("/src/lib/offline-cache.ts").then(({ openOfflineCache }) =>
        openOfflineCache({ userId: "manual-tab-alice-7", scope: fixture.aliceScope as never }).then((v) => { v.close(); return false; }, () => true),
      );

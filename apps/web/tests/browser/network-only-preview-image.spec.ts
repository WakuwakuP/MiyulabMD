...
test("cache-enabled authenticated network preview remains cache-backed", async ({
  page,
}) => {
  const result = await page.evaluate(async () => {
    const { resolvePreviewImages } = await import("/src/lib/preview-images.ts");
    return resolvePreviewImages(
      '<img src="/api/notes/note-1/images/image-1">',
      { enabled: true, urls: new Map() },
    );
  });
  expect(result).not.toContain('src="/api/notes/note-1/images/image-1"');
});

test("foreground and background image sharing keeps one successful cache result", async ({
  page,
}) => {
  const result = await page.evaluate(async () => {
    const { resolvePreviewImages } = await import("/src/lib/preview-images.ts");
    return resolvePreviewImages(
      '<img src="/api/notes/note-1/images/image-1">',
      { enabled: true, urls: new Map([["/api/notes/note-1/images/image-1", "blob:shared"]]) },
    );
  });
  expect(result).toContain("blob:shared");
});

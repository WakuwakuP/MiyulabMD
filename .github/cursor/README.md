...
repository. It upgrades pnpm dependencies on a weekly schedule and opens a pull
request after the same checks as the PR gates, plus typecheck and a production
build (`pnpm check`, `pnpm typecheck`, `pnpm test`, `pnpm build`).
...
`Shell(pnpm)` — not `Shell(true)` or `Shell(*)`.
